...
type UrlaubsBuchung = Database["public"]["Tables"]["urlaubs_buchungen"]["Row"];
...
export function AdminUrlaubsKonten() {
  ...
  const [buchungen, setBuchungen] = useState<UrlaubsBuchung[]>([]);
  ...
  const load = async () => {
    ...
    ((sal as any[]) ?? []).forEach((r) => {
      s[r.mitarbeiter_id] = Number(r.saldo_tage ?? 0);
  ...
  const loadBuchungen = async (uid: string) => {
    const { data } = await supabase
      ...
      .limit(200);
    setBuchungen((data as UrlaubsBuchung[]) ?? []);
  };
  ...
  const fillUp = async () => {
    const { data, error } = await supabase.rpc("urlaub_gutschriften_nachholen" as any);
    if (error) {
      toast({ variant: "destructive", title: "Fehler", description: error.message });
      return;
    }
    toast({
      title: `${(data as any[])?.length ?? 0} Gutschriften gebucht`,
    });
    load();
    ...
    <div className="space-y-3">
      ...
      <Card>
        <CardContent className="p-3 flex items-center gap-2 flex-wrap">
          ...
          <span className="text-sm font-semibold">Urlaubs-Konten</span>
                  ...
                  <tr key={p.id} className="border-t hover:bg-muted/30 cursor-pointer"
                    ...
                    <td
                      className={`px-3 py-2 text-right font-bold tabular-nums ${
                        sal < 0 ? "text-red-700" : sal < 5 ? "text-amber-700" : ""
                      }`}
                    >
                      {fmtTage(sal)}
                    </td>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && closeDetail()}>
        <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col p-0">
          <DialogHeader className="px-4 pt-4 pb-2">
            <DialogTitle>
              Urlaubs-Konto: {selected?.vorname} {selected?.nachname}
            </DialogTitle>
            <div className="text-sm text-muted-foreground">
              Aktueller Saldo:{" "}
              <strong className="text-foreground">
                {fmtTage(selected ? salden[selected.id] ?? 0 : 0)}
              </strong>
            </div>
          </DialogHeader>
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted">
                <tr>
                  ...
                  <th className="text-right px-2 py-1">Tage</th>
              ...
              <tbody>
                {buchungen.map((b) => (
                  <tr key={b.id} className="border-t">
                    ...
                    <td className="px-2 py-1">{URLAUB_ART_LABEL[b.art] ?? b.art}</td>
                    <td
                      className={`px-2 py-1 text-right tabular-nums font-semibold ${
                        Number(b.tage) < 0 ? "text-red-700" : "text-emerald-700"
                      }`}
                    >
                      {Number(b.tage) > 0 ? "+" : ""}
                      {Number(b.tage).toFixed(2).replace(".", ",")}
                    </td>
...
function NewUrlaubBuchungDialog({
  open,
  onOpenChange,
  profile,
  onSaved,
  ...
  const { toast } = useToast();
  const [art, setArt] = useState<UrlaubsBuchungArt>("korrektur");
  ...
  const save = async () => {
    if (!profile) return;
    const t = Number(tage.replace(",", "."));
    if (!Number.isFinite(t) || t === 0) {
      toast({ variant: "destructive", title: "Tage-Wert ungültig" });
      return;
    }
    const { data: u } = await supabase.auth.getUser();
    const { error } = await supabase.from("urlaubs_buchungen").insert({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Manuelle Urlaubs-Buchung</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div>
            ...
            <select
              value={art}
              onChange={(e) => setArt(e.target.value as UrlaubsBuchungArt)}
              className="w-full h-9 rounded-md border bg-background px-2 text-sm"
            >
              <option value="initial">Initial-Saldo</option>
              <option value="jahresgutschrift">Jährliche Gutschrift</option>
              <option value="monatsgutschrift">Monatliche Gutschrift</option>
